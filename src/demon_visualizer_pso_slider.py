...
STYLE = {
    ...
    'spectrum_lw': 0.8,          # Spectrum line width (thinner)
    'marker_lw': 1.0,            # Harmonic marker line width
    'font_size': 12,
    'title_size': 14,
    'label_size': 11,
}
...
def compute_demon_spectrum(signal, fs, bandpass_low=500, bandpass_high=1500, 
    ...
    4. AC coupling (remove DC) + high-pass to remove slow drift
    ...
    hp_cutoff : float
        High-pass cutoff for envelope to remove slow drift (Hz, default: 0.3)
        Set to 0 to disable. Keeps SR/BR intact (typically > 1 Hz)
    ...
    # Step 4: AC coupling + remove near-DC drift
    envelope_ac = envelope - np.mean(envelope)
    
    # High-pass filter to remove slow drift/swell (keeps SR/BR intact)
    if hp_cutoff > 0:
    ...
    # Step 5: FFT with zero-padding for frequency resolution
...
def plot_demon_spectrum(freqs, spectrum_db, blade_rate=None, shaft_rate=None, n_harmonics=4,
                        ...
                        figsize=(14, 7), dpi=150, smooth_display=0, interactive=True):
    """
    Create professional DEMON spectrum plot with optional interactive smoothing slider.
    ...
    smooth_display : int
        Initial smoothing window size (0=off, try 3-15 for smoother look)
    interactive : bool
        Add interactive smoothing slider (default: True, disabled if output_path set)
    ...
    # Disable interactive mode if just saving
    if output_path and not show:
        interactive = False
    
    # Create figure with extra space for slider if interactive
    if interactive and show:
        fig, ax = plt.subplots(figsize=figsize, facecolor=STYLE['bg_color'])
        plt.subplots_adjust(bottom=0.18)  # Make room for slider
    else:
        fig, ax = plt.subplots(figsize=figsize, facecolor=STYLE['bg_color'])
    ...
    # Store marker elements for potential updates
    sr_lines = []
    sr_texts = []
    br_lines = []
    br_texts = []
        ...
        for i in range(1, n_harmonics + 1):
            ...
            if harmonic_freq <= freqs[-1]:
                ...
                if not is_br_harmonic:
                    vline = ax.axvline(x=harmonic_freq, color=sr_color,
                                       linestyle=':', linewidth=1.0,
                                       alpha=0.7, zorder=4)
                    sr_lines.append(vline)
                    
                    # Label ABOVE the peak
                    peak_db = get_db_at_freq(harmonic_freq, display_db)
                    label_y = min(peak_db + 4, 3)
                    txt = ax.text(harmonic_freq, label_y, f'{i}×SR\n({harmonic_freq:.1f}Hz)',
                                  color=sr_color, fontsize=8,
                                  ha='center', va='bottom', fontweight='bold')
                    sr_texts.append((txt, harmonic_freq, i))
    
    # Add blade rate harmonic markers if specified (thinner lines)
    if blade_rate is not None and blade_rate > 0:
        for i in range(1, n_harmonics + 1):
            harmonic_freq = blade_rate * i
            if harmonic_freq <= freqs[-1]:
                vline = ax.axvline(x=harmonic_freq, color=STYLE['marker_color'],
                                   linestyle='--', linewidth=1.0,
                                   alpha=0.7, zorder=4)
                br_lines.append(vline)
                
                # Label ABOVE the peak
                peak_db = get_db_at_freq(harmonic_freq, display_db)
                label_y = min(peak_db + 4, 3)
                txt = ax.text(harmonic_freq, label_y, f'{i}×BR\n({harmonic_freq:.0f}Hz)',
                              color=STYLE['marker_color'], fontsize=9,
                              ha='center', va='bottom', fontweight='bold')
                br_texts.append((txt, harmonic_freq, i))
...
def analyze_and_plot(audio_path, fs=16000, bandpass_low=500, bandpass_high=1500,
                     ...
                     show=True, verbose=True, auto_detect=True, smooth_display=0):
    """
    ...
    smooth_display : int
        Moving average window for display smoothing (0=off)
...
Vessel Type Presets (--vessel):
  ...
  default   500-1500 Hz    General purpose
    ...
    parser.add_argument('--smooth', type=int, default=0,
                        help='Display smoothing window size (0=off, try 3-7 for smoother look)')
